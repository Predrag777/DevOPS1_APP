...
export const getPatients = async (req: Request, res: Response): Promise<Response<Patient[]>> => {
    ...
    try{
        ...
        return res.status(Code.OK)
            .send(new HttpResponse(Code.OK, Status.OK, 'Patients retrieve'))
    } catch(error: unknown){
        console.error(error);
        return res.status(Code.INTERNAL_SERVER_ERROR)
            .send(new HttpResponse(Code.INTERNAL_SERVER_ERROR, Status.INTERNAL_SERVER_ERROR, "Error occured"));
    }
}
...
export const getPatient=async(req:Request, res:Response): Promise<Response<Patient[]>> =>{
    try{
        ...
        if((result[0] as Array<ResultSet>).length>0){
            return res.status(Code.OK)
                .send(new HttpResponse(Code.OK, Status.OK, 'One patient retreived'))
        }else{
...
export const createPatient=async(req:Request, res:Response): Promise<Response<Patient[]>> =>{
    ...
    try{
        ...
        if((result[0] as Array<ResultSet>).length>7){
            return res.status(Code.OK)
                .send(new HttpResponse(Code.OK, Status.CREATED, 'One patient created'))
        }else{
            return res.status(Code.BAD_REQUEST)
            .send(new HttpResponse(Code.BAD_REQUEST, Status.BAD_REQUEST, 'Not found'))
        }
